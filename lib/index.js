      ...
      if (isRule(topKey)) {
        switch (rules[key]) {
          ...
          case 'replace':
            ...
            return b;
          case 'match-replace':
            return [].concat(_toConsumableArray(replaceMatch(a, b)), _toConsumableArray(removeMatch(b, a)));
          default:
            // append
...
function replaceMatch(a, b) {
  return a.map(function (seenRule) {
    var match = b.find(function (newRule) {
...
function removeMatch(a, b) {
  return a.filter(function (seenRule) {
    return !b.find(function (newRule) {
      return seenRule.test.toString() === newRule.test.toString();
    });
  });
}
